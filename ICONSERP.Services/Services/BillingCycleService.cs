...
{
    public class BillingCycleService : GenericService<BillingCycle, CountryEditViewModel, CountryViewModel>, IBillingCycleService
    {
        ...
        public bool IsExist(string nameArabic, string nameEnglish ,int DisplayOrder)
        {
            var query = _unitOfWork.BillingCycleRepository.GetAll();
            if (int.Parse(DisplayOrder.ToString()) > 0)
                query = query.Where(i => i.DisplayOrder != DisplayOrder);
            return query.Any(x => x.NameArabic == nameArabic || x.NameEnglish == nameEnglish || x.DisplayOrder == DisplayOrder);
        }
